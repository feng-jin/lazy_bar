...
- 当前是单 target 的 macOS 原生应用，菜单栏入口和设置页都基于 SwiftUI；菜单栏使用 `MenuBarExtra`，设置窗口通过显式设置打开动作唤起。
- 当前只使用 mock 数据，尚未接入真实行情。
- 当前菜单栏可展示股票代码、股票简称、最新价和涨跌幅，展示字段可在设置中勾选，并通过保存/取消按钮提交或放弃修改。
- 点击菜单栏后会展开自定义 SwiftUI 下拉面板，当前提供带小图标的“设置 Settings”和“退出 Quit Lazy Bar”操作；面板里的每一行都支持整行点击与 hover 高亮。
