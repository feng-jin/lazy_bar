...
- 当前菜单栏可展示股票代码、股票简称、最新价和涨跌幅，展示字段可在独立设置弹窗中通过带说明的两列卡片式选项勾选；设置弹窗已拆成“监控股票”和“展示字段”两个 tab，其中“监控股票”tab 收敛为单段式编辑区：顶部一行录入简称与代码，下面紧接固定高度的可编辑列表，支持手动新增、删除和直接编辑股票代码/简称；展示字段则单独放在第二个 tab 中，并要求至少保留一个字段可见，避免 bar 和主面板进入空展示状态。每次点击设置打开弹窗时，默认都会回到“监控股票”tab。左键主面板中的股票区域在达到上限后会滚动，避免占用过大空间。点击保存会立即提交但不关闭弹窗，点击取消会放弃本轮未保存修改并关闭弹窗；如果这次保存只改了展示字段或股票简称、没有改 symbol 列表，bar 和左键主面板会直接基于现有行情快照重算，不再等下一次拉行情；如果改了 symbol 列表，则会先保留当前仍有效的已加载行情并立即后台拉取新列表，避免 bar 退回成启动时的“加载中...”空态。bar ticker 在展示数据或顺序变化时会按新列表重启动画，确保与左键列表按同一份最新数据同步切换。
