...
- 当前菜单栏可展示股票代码、股票简称、最新价和涨跌幅，展示字段可在独立设置弹窗中通过带说明的两列卡片式选项勾选；设置弹窗已拆成“监控股票”和“展示字段”两个 tab，其中股票配置区继续使用新增卡片与列表卡片两段结构，支持手动新增、删除和直接编辑股票代码/简称，展示字段则单独放在第二个 tab 中；已录入股票列表区保持固定高度，避免条目数量变化时整体布局跳动。左键主面板中的股票区域在达到上限后会滚动，避免占用过大空间。点击保存会立即提交但不关闭弹窗，点击取消会放弃本轮未保存修改并关闭弹窗。
