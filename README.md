...
- 当前是单 target 的 macOS 原生应用，设置页基于 SwiftUI；菜单栏入口通过 `NSStatusItem` 承载，设置窗口通过显式设置打开动作唤起。
- 当前只使用 mock 数据，尚未接入真实行情；当前 mock 链路已提供多只示例股票。
- 当前菜单栏可展示股票代码、股票简称、最新价和涨跌幅，展示字段可在设置中勾选，并通过保存/取消按钮提交或放弃修改。
- 左键点击菜单栏会弹出股票列表，右键点击会显示“设置 Settings”和“退出 Quit Lazy Bar”操作菜单；bar 顶部仍显示第一只股票的紧凑摘要。
