...
- 当前只使用 mock 数据，尚未接入真实行情；当前 mock 链路已提供多只示例股票，并会按固定节奏重新拉取、生成小幅波动的数据来模拟准实时刷新。
- 当前菜单栏可展示股票代码、股票简称、最新价和涨跌幅，展示字段可在设置中勾选，并通过保存/取消按钮提交或放弃修改。
- 左键点击菜单栏会从状态栏图标下方展开无箭头的股票列表面板；面板使用更贴近 macOS 原生菜单的材质、圆角边界、分隔线与 hover 高亮，并优先与 bar 使用同一宽度和左右边界对齐；bar 与左键列表会共享同一套列结构、列间距、主副文字体风格、数值字体风格，以及按当前股票里最长字段动态计算出的列宽，因此股票简称列、股价列、涨跌幅列都能对齐，像四位数股价这类更长文本也会带着整列一起扩展；左键面板高度会按当前股票个数和实际内容高度自适应，超过上限后再进入滚动；若系统可用宽度仍不足，则会优先压缩股价列，再尽量保留股票简称和涨跌幅；右键点击会从状态栏图标下方展开“设置 Settings”和“退出 Quit Lazy Bar”系统菜单。
