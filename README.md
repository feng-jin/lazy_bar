...
- 当前默认通过新浪财经未公开快照接口拉取 A 股准实时行情，并继续按固定节奏轮询刷新；仓库仍保留 mock 行情 provider 供 Preview 和后续兜底使用。watchlist 不再依赖项目内 JSON 资源，首次启动默认为空列表，由用户在设置页逐条手动录入并持久化。
- 首次启动且 watchlist 为空时，菜单栏会直接显示“请先添加股票”作为空态提示，避免状态栏 item 因无数据而看起来像没有启动。
- 当前菜单栏可展示股票代码、股票简称、最新价和涨跌幅，展示字段可在独立设置弹窗中通过带说明的两列卡片式选项勾选；设置弹窗已拆成“监控股票”和“展示字段”两个 tab，其中“监控股票”tab 收敛为单段式编辑区：顶部一行录入简称与代码，下面紧接固定高度的可编辑列表，支持手动新增、删除和直接编辑股票代码/简称；展示字段则单独放在第二个 tab 中。左键主面板中的股票区域在达到上限后会滚动，避免占用过大空间。点击保存会立即提交但不关闭弹窗，点击取消会放弃本轮未保存修改并关闭弹窗。
