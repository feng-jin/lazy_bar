...
- 左键点击菜单栏会从状态栏图标下方展开无箭头的股票列表面板；面板使用更贴近 macOS 原生菜单的材质、圆角边界、分隔线与 hover 高亮，并优先与 bar 使用同一宽度和左右边界对齐；bar 与左键列表会共享同一套列结构、列间距、主副文字体风格、数值字体风格，以及按当前股票里最长字段动态计算出的列宽，因此股票简称列、股价列、涨跌幅列都能对齐，像四位数股价这类更长文本也会带着整列一起扩展；若系统可用宽度仍不足，则会优先压缩股价列，再尽量保留股票简称和涨跌幅；右键点击会从状态栏图标下方展开“设置 Settings”和“退出 Quit Lazy Bar”系统菜单。
