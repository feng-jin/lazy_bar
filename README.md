...
- 左键点击菜单栏会从状态栏图标下方展开无箭头的股票列表面板；列表文案与 bar 上当前 ticker 使用同一套字段配置、顺序与文本拼接规则，并在打开期间与 bar 保持同步变化；右键点击会从状态栏图标下方展开“设置 Settings”和“退出 Quit Lazy Bar”系统菜单；bar 顶部使用固定宽度 ticker 上下循环播放股票摘要，避免因文本长短不一导致状态栏宽度跳变。
