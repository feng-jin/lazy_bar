...
9. `StatusBarController` 将 `MenuBarLabelView` 托管到 `NSStatusBarButton` 内部，并根据 `MenuBarPresentationStore` 当前 presentation 中的 layout 同步调整状态栏按钮宽度；宽度更新只调整宿主 `NSStatusItem`，不会在每次行情刷新时重建整棵 SwiftUI ticker 根视图，避免纵向滚动动画和内部 `@State` 周期性被重置。`MenuBarLabelView` 在裁剪容器里按条目做纵向循环滚动，并使用共享列行组件对齐渲染股票简称、股价与涨跌幅；左键点击后展示的主面板直接复用同一份 presentation，上半部分继续共享相同的分栏展示数据、动态列宽与共享样式 token，下半部分只承载设置入口、退出和后续少量操作。
