...
- `ViewModels`
  - `MenuBarViewModel`：负责菜单栏 ticker 数据源、下拉股票列表状态，以及定时刷新调度；通过独立的 `QuoteRefreshScheduler` 计算 A 股交易时段下的刷新节奏；内部显式维护 `loading / emptyWatchlist / failed / loaded` 状态，并通过单一的 settings 变更入口区分“symbol 列表变化”和“仅展示配置变化”，再把内容状态映射给 builder 生成 `MenuBarPresentation`。
