...
- `MenuBarDisplaySettings` 和 `MenuBarSettingsStore` 负责展示配置、watchlist 配置与持久化；watchlist 不再依赖 bundle 内置 JSON，首次启动为空列表，后续由用户手动维护。`MenuBarSettingsViewModel` 负责设置弹窗中的草稿、股票代码编辑、校验，以及保存/取消动作，避免把设置状态散落在 View 里。
