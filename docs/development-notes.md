...
- 当前状态栏标题会按顺序轮播所有股票摘要；左键点击后展示股票列表弹层，每项展示代码、名称、价格和涨跌幅；右键点击后展示设置和退出系统菜单。
...
- 要调整菜单内容或股票列表项：优先查看 `Sources/Views/MenuBar/MenuBarContentView.swift`、`Sources/ViewModels/MenuBarViewModel.swift` 和 `Sources/Models/DisplayQuote.swift`。
