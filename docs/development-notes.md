...
- 当前设置能力通过独立设置弹窗承载，不再把设置表单直接内嵌到左键主面板里；展示字段编辑与 watchlist 维护继续通过 `MenuBarSettingsViewModel` 管理草稿、行内编辑、base 列表恢复、保存前校验、保存和取消。设置窗口内容区由显式约束填满窗口的 `NSHostingView` 承载，并在展示前按 SwiftUI 内容高度同步窗口尺寸，避免弹窗打开但内容空白。
- 设置页支持勾选菜单栏展示字段，并基于项目内的 base watchlist 做手动增删改查；watchlist 区域采用一行一只股票的紧凑编辑列表，只保留必要输入和删除操作；列表本身会在固定高度内滚动，避免设置窗口随着 watchlist 变长持续增高；保存会立即写回持久化配置但保留弹窗，取消会放弃本轮未保存草稿并关闭窗口。
