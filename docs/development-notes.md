...
- 当前共享列宽测量与行渲染已经从 `MenuBarViewModel` 抽离到菜单栏视图层辅助对象中，`MenuBarViewModel` 主要负责数据拉取、刷新、`DisplayQuote` 生成，以及 `loading / emptyWatchlist / failed / loaded` 的显式状态协调，避免继续耦合 `AppKit` 字体测量细节，也避免把状态语义散落到 View 里推断。
