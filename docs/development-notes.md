...
- 搜索链路现在补了更细的调试日志：`MenuBarSettingsViewModel` 会记录防抖后的 query、旧任务取消、新任务开始、搜索完成结果数和失败原因；设置页股票检索 provider 会记录请求 URL、HTTP 状态、解码编码、原始候选数、过滤后候选数以及解析失败原因；`SettingsWindowController` 还会在搜索结果、搜索中状态和搜索提示文案变化时记录 `hostingView.fittingSize`、窗口 content/frame 尺寸以及下拉层显隐状态，方便判断问题发生在请求链路还是下拉层布局链路。
