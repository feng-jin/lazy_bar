...
- 当前共享列宽测量与行渲染已经从 `MenuBarViewModel` 抽离到菜单栏视图层辅助对象中，A 股交易时段对应的刷新间隔计算也已下沉到独立的 `QuoteRefreshScheduler`；`MenuBarViewModel` 主要负责数据拉取、刷新、`DisplayQuote` 生成，以及 `loading / emptyWatchlist / failed / loaded` 的显式状态协调。当 watchlist 在加载过程中被修改时，旧请求会被取消，只有当前仍被跟踪的请求结果允许回写 UI，避免过期行情重新出现在 bar 或主面板里。这样既避免继续耦合 `AppKit` 字体测量细节，也避免把状态语义散落到 View 里推断。
- 当前设置能力通过独立设置弹窗承载，不再把设置表单直接内嵌到左键主面板里；展示字段编辑与 watchlist 维护继续通过 `MenuBarSettingsViewModel` 的单一草稿模型管理行内编辑、保存前校验、保存和取消。设置窗口内容区由显式约束填满窗口的 `NSHostingView` 承载，并在展示前按 SwiftUI 内容高度同步窗口尺寸，避免弹窗打开但内容空白；当前窗口宽度已放宽，并将设置内容拆成两个 tab，以避免长表单继续把底部操作区挤出可视范围。
...
- 要调整主面板内容或股票列表项：优先查看 `Sources/App/StatusBarController.swift`、`Sources/Views/MenuBar/QuotesPopoverView.swift`、`Sources/Views/MenuBar/MenuBarLabelView.swift`、`Sources/Views/MenuBar/QuoteColumnLayout.swift`、`Sources/Views/MenuBar/QuoteColumnsRowView.swift`、`Sources/ViewModels/MenuBarViewModel.swift` 和 `Sources/Models/DisplayQuote.swift`。
