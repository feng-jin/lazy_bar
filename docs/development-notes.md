...
- 当前共享列宽测量与行渲染已经从 `MenuBarViewModel` 抽离到菜单栏视图层辅助对象中，由 `MenuBarViewModel` 自己在监听 `settingsStore` 与内部 `viewState` 后直接产出 `MenuBarPresentation`，供 bar ticker、左键股票列表和状态栏宽度共用同一份 rows / layout / statusText；A 股交易时段对应的刷新间隔计算也已下沉到独立的 `QuoteRefreshScheduler`。`MenuBarViewModel` 主要负责数据拉取、刷新、`DisplayQuote` 生成，以及 `loading / emptyWatchlist / failed / loaded` 的显式状态协调，并额外保留一份最近成功拉取的 `[symbol: StockQuote]` 快照：当设置只改了展示字段或股票简称时，会直接基于这份快照重算展示，不再等下一次行情刷新；只有 symbol 列表变化时才重新触发拉取。当 watchlist 在加载过程中被修改时，旧请求会被取消，只有当前仍被跟踪的请求结果允许回写 UI，避免过期行情重新出现在 bar 或主面板里。左键股票列表会直接观察同一个 `MenuBarViewModel.presentation`；bar 则由 `StatusBarController` 在 `presentation` 变化时直接替换一次 `MenuBarLabelView` 的 `rootView` 快照，并同步更新宽度和尺寸，以降低“左键列表已经显示数据，但 bar 仍停在加载中”以及展示字段配置切换后两边短暂不一致的概率。共享列宽测量当前要求与 SwiftUI 实际渲染使用同一套字体语义和字号，避免字体测量与真实显示不一致导致的列宽误差。这样既避免继续耦合 `AppKit` 字体测量细节，也减少了展示状态在多层对象之间转发。
