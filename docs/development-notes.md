...
- 当前状态栏使用 SwiftUI ticker view 按股票上下循环播放摘要；ticker 与左键主面板上半部分的股票列表会共享同一套列结构、主副文字体风格、数值字体风格，以及按当前股票里最长字段动态计算出的列宽，因此股票简称、股票代码、股价、涨跌幅都能整列对齐并完整显示，遇到更长的文本时，bar 和左键面板会一起扩展宽度，而不是用 `...` 截断；关闭部分字段后，bar 和左键面板也会随剩余列一起收紧，不再保留固定最小宽度。当前 bar 与左键列表中的股票简称、股票代码、股价、涨跌幅均统一为 12pt；四个字段当前统一按各自列宽右对齐，以便让字段尾部边界更整齐；其中股票简称与股票代码共用同一套主标识颜色和字重，避免视觉节奏不一致，同时身份列到股价列之间会使用比其余列更宽的前导间距，避免价格位数变化时名称与股价看起来贴得过近。价格和涨跌幅列在 SwiftUI 渲染与 AppKit 宽度测量中统一使用 monospaced digit 语义，避免位数变化或正负号切换时出现列宽抖动；`StatusBarController` 在 `presentation` 变化时会同步更新 `NSStatusItem` 宽度、状态栏 hosting view 尺寸，以及已打开主面板的容器宽高，让 bar、popover 与内部股票列表持续共用同一份 presentation/layout 语义；当展示 rows 发生变化时，ticker 会按新列表重启，避免 bar 与左键列表短暂停留在不同顺序。左键点击后会从状态栏图标下方展开无箭头的主面板，其容器材质、圆角、分隔线和 hover 态已向 macOS 原生菜单观感收敛，并与状态栏按钮保持同一宽度；股票列表与底部操作区共享同一套行内边距、分隔线起点和紧凑纵向节奏，减少视觉断层；面板外层仅保留纵向留白，因此股票列表内容的左右起止位置会与 bar 对齐；股票展示区达到固定高度上限后会滚动，避免 watchlist 增长时继续撑高面板。
- 当前共享列宽测量与行渲染已经从 `MenuBarViewModel` 抽离到菜单栏视图层辅助对象中；`MenuBarPresentation` 现在保持纯展示模型，只接收 `DisplayQuote + settings + statusText`，不再直接依赖某个具体 ViewModel 状态类型。`MenuBarViewModel` 与 `MenuBarPresentationBuilder` 现在共用同一份 `MenuBarContentState`，统一表达 `loading / emptyWatchlist / failed / loaded([DisplayQuote])`，不再维护重复的 `ViewState / ContentState` 枚举与映射胶水。`MenuBarViewModel` 主要负责 settings 订阅、UI 状态协调和 `DisplayQuote` 生成；行情拉取、请求取消、最近成功快照缓存、交易时段轮询刷新，以及刷新失败时优先保留最后一次有效快照，都已下沉到独立的 `QuoteSession`，后者继续通过 `QuoteRefreshScheduler` 计算 A 股交易时段下的刷新间隔。settings 变更仍经由单一入口统一判断是“symbol 列表变化”还是“仅展示配置变化”，前者才触发重新拉取，后者直接基于现有快照重算展示，减少重复的状态分支与订阅胶水。当 watchlist 在加载过程中被修改时，旧请求会被取消，只有当前仍被跟踪的请求结果允许回写 UI，避免过期行情重新出现在 bar 或主面板里；当 symbol 列表变化时，session 会先裁剪到仍然有效的旧快照，再由 ViewModel 决定是否继续保持已加载展示或回到 loading。左键股票列表会直接观察同一个 `MenuBarViewModel.presentation`；bar 则由 `StatusBarController` 在 `presentation` 变化时直接消费 sink 传入的新值，同步状态栏宿主和已打开面板的尺寸，以降低“左键列表已经显示数据，但 bar 仍停在加载中”以及展示字段配置切换后两边短暂不一致的概率。共享列宽测量当前要求与 SwiftUI 实际渲染使用同一套字体语义和字号，避免字体测量与真实显示不一致导致的列宽误差。这样既避免继续耦合 `AppKit` 字体测量细节，也减少了展示状态在多层对象之间转发。
...
- 要调整状态栏标题、ticker 动画或点击行为：优先查看 `Sources/App/StatusBarController.swift`、`Sources/Views/MenuBar/MenuBarLabelView.swift`、`Sources/Views/MenuBar/MenuBarPresentation.swift`、`Sources/Views/MenuBar/MenuBarStyle.swift`、`Sources/ViewModels/MenuBarContentState.swift`、`Sources/ViewModels/MenuBarPresentationBuilder.swift`、`Sources/ViewModels/MenuBarViewModel.swift` 和 `Sources/Models/DisplayQuote.swift`。
- 要调整主面板内容或股票列表项：优先查看 `Sources/App/StatusBarController.swift`、`Sources/Views/MenuBar/QuotesPopoverView.swift`、`Sources/Views/MenuBar/MenuBarPresentation.swift`、`Sources/Views/MenuBar/MenuBarStyle.swift`、`Sources/Views/MenuBar/QuoteColumnLayout.swift`、`Sources/Views/MenuBar/QuoteColumnsRowView.swift`、`Sources/ViewModels/MenuBarContentState.swift`、`Sources/ViewModels/MenuBarPresentationBuilder.swift`、`Sources/ViewModels/MenuBarViewModel.swift`、`Sources/ViewModels/QuoteSession.swift` 和 `Sources/Models/DisplayQuote.swift`。
