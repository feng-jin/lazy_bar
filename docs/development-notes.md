...
- 设置页当前分成“监控股票”和“展示字段”两个 tab；watchlist tab 中继续使用“新增股票”卡片和“已录入股票”列表卡片，列表带表头、计数胶囊和独立行卡片，只保留必要输入和删除操作，且整块列表区域保持固定高度，避免条目数量变化时卡片尺寸波动；展示字段 tab 中使用两列卡片式勾选项，每个字段补充用途说明，减少原先一行一个 `Toggle` 的冗长感；列表本身会在固定高度内滚动，底部保存和取消区域固定在窗口底部，避免随着上方内容变长而消失；保存会立即写回持久化配置但保留弹窗，取消会放弃本轮未保存草稿并关闭窗口。
