...
- 当前设置能力通过独立设置弹窗承载，不再把设置表单直接内嵌到左键主面板里；展示字段编辑与 watchlist 维护继续通过 `MenuBarSettingsViewModel` 管理草稿、行内编辑、保存前校验、保存和取消。设置窗口内容区由显式约束填满窗口的 `NSHostingView` 承载，并在展示前按 SwiftUI 内容高度同步窗口尺寸，避免弹窗打开但内容空白；当前窗口宽度已放宽，以容纳更高信息密度的字段选择区。
- 设置页支持勾选菜单栏展示字段，并通过手动录入方式维护 watchlist；watchlist 区域当前分成“新增股票”卡片和“已录入股票”列表卡片，列表带表头、计数胶囊和独立行卡片，只保留必要输入和删除操作；展示字段区域改为两列卡片式勾选项，每个字段补充用途说明，减少原先一行一个 `Toggle` 的冗长感；列表本身会在固定高度内滚动，避免设置窗口随着 watchlist 变长持续增高；保存会立即写回持久化配置但保留弹窗，取消会放弃本轮未保存草稿并关闭窗口。
